...
//Note from Robin: I found this implementation from the text book here:
//https://algs4.cs.princeton.edu/32bst/BST.java.html
//I've removed a number of methods that weren't relevant to the project
//so the file's less crowded and it's easier to find my method.
//I created containsWithCompares and its helper methods.
...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    //Robin's implementation/alteration of "contains" that returns instead the number of compares.
    public int containsWithCompares(Key key) {
        if (key == null) throw new IllegalArgumentException("argument to contains() is null");
        return getCompares(key);
    }

    private int getCompares(Key key) {
        int[] array = {0};
        getCompares(root, key, array);
        return array[0];
    }

    private Value getCompares(Node node, Key key, int[] array) {
        if (node == null) {
            return null;
        }
        array[0]++;
        ...
        if (cmp < 0) {
            return getCompares(node.left, key, array);
        }
        else if (cmp > 0) {
            return getCompares(node.right, key, array);
        }
        else {
            return null;
        }
